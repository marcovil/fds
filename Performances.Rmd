...
Creo un tibble con solo i concerti. Mi concentro solo sull'attività della New York Philarmonic.

```{r}
concerts <- ccseason %>%
  filter(orchestra == "New York Philharmonic") %>%
  distinct(concertID, Date, year, season, seasonN, eventType)
```
...
Limitandomi al periodo pre-covid.
...
Che tipi di concerti ci sono. Mi limito alle categorie che contengono più di 100 concerti.
...
```
Fra gli eventi che non fanno parte della stagione regolare, quelli più presenti con maggior continuità sono i tour.
...
Da ora in poi mi limito a lavorare sulla Subscription Season della New York Philharmonic.
...
Come si è evoluto nelle stagioni il numero di rappresentazioni per compositore.
