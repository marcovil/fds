...
# lascio Time com'è, sistemo la data, tenendo solo anno, mese, giorno, aggiungo seasonStart (come tipo intero)
programsT <- programsT %>%
...
```{r}
#trovo i compositori che compaiono in un solo anno per poi toglierli
joy <- programsT %>%
  distinct(eventType,Location,Venue,Date,Year,Time,workID,composerName) %>%
  filter(!is.na(composerName)) %>%
  group_by(composerName) %>%
...
ggplot(models, mapping = aes(x = Year, y = .fitted, color = composerName)) +
  geom_line(alpha = 1/2, show.legend = FALSE)
