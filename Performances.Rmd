...
Quali sono gli eventType preponderanti dal 2000 in poi non di tipo Subscription Season?
...
Un altro subset, compositori rappresentati prima della stagione 1909, per almeno 10 stagioni. Rappresentano il 43 % delle performance totali.
...
Guardo ai compositori che hanno residui più lontani di 0.1 da 0.
...
Quelli che sono stati rappresentati prima della stagione 1909-1910 per almeno dieci stagioni.
...
pcp %>%
  ...
  geom_vline(aes(xintercept = 1909, color = "red")) +
...
regs <- ccn2_nested %>%
  mutate(
    ...
    model2 = map(data, ccn2_lin),
    tidied = map(model, tidy),
    tidied2 = map(model2, tidy),
    glanced = map(model, glance),
    glanced2 = map(model2, glance),
    augmented = map(model, augment),
    augmented2 = map(model2, augment),
  )
...
performances %>%
  ...
  filter(composerName == "Berlioz, Hector")
```
...
very_good <- regs %>%
  ...
  mutate(very_good = TRUE) %>%
select(conductorName, composerName, very_good)
...
Guardo a quelli più alti di 0.2
...
Direttori-compositori con fit molto buono per coppie che compaiono almeno per 3 stagioni.
...
ccn2 %>%
  ...
  mutate(seasons = n_distinct(seasonYear)) %>%
  filter(seasons > 3) %>%
  inner_join(very_good) %>%
  ggplot() +
  geom_line(aes(seasonYear,perc,color=conductorName), show.legend = TRUE) +
  geom_point(aes(seasonYear,perc,color=conductorName), show.legend = FALSE)
```

Conduttori con modelli molto buoni.
...
condover3 <- ccn2 %>%
  ...
  group_by(conductorName) %>%
  mutate(
    models = n_distinct(composerName),
  ) %>%
  filter(very_good) %>%
  mutate(
    very_good_models = n_distinct(composerName),
    perc_very_good = very_good_models / models
  ) %>%
  filter(perc_very_good > 0.4 & perc_very_good < 1) %>%
  distinct(conductorName)
```

Bernstein, fit molto buono.
...
ccn2 %>%
  ...
  mutate(seasons = n_distinct(seasonYear)) %>%
  filter(seasons > 1) %>%
  filter(conductorName == "Busch, Fritz") %>%
  inner_join(very_good) %>%
  ggplot() +
  geom_line(aes(seasonYear,perc,color=composerName), show.legend = FALSE) +
  geom_point(aes(seasonYear,perc,color=composerName), show.legend = FALSE) +
...
regs %>%
  ...
  filter(estimate < 0.5 & estimate > -0.5)

regs %>%
  unnest(tidied) %>%
  filter(term == "seasonYear") %>%
  ggplot() +
  geom_histogram(aes(estimate, binwidth = 1)) +
