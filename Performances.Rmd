...
performances %>%
  ...
  coord_cartesian(xlim = c(1840,1940))
```
...
```{r}
pcp <- performances %>%
  group_by(seasonYear,seasonN,composerName) %>%
  count() %>%
  group_by(seasonYear,seasonN) %>%
...
Un altro subset, compositori rappresentati prima della stagione 1909, per almeno 70 stagioni con densità 50 %. Rappresentano il 76 % delle performance.
...
Modelli in cui la componente quadratica è positiva e la componente lineare è negativa. Primi 30 per componente quadratica.
...
Quelli che sono stati rappresentati prima della stagione 1909-1910.
...
pcp %>%
  ...
  arrange(-perc) %>%
  mutate(
    rank = row_number(perc)
  ) %>%
  arrange(seasonN) %>%
  filter(rank <= 2) %>%
  ggplot() +
  geom_point(aes(seasonN,perc,color=composerName), show.legend = FALSE)
```
...
glance <- ccn_nested_models_glance %>%
  unnest(glance)
...
performances %>%
  ...
  mutate(totalC = n()) %>%
  mutate(percent = totalC / totalM) %>%
  filter(composerName == "Tchaikovsky, Pyotr Ilyich") %>%
  ggplot() +
  geom_bar(aes(x=month,fill=eventType))
```
