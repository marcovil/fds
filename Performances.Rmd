...
Questo fatto con le percentuali. Per ogni compositore, la percentuale di performance del compositore rispetto al totale della stagione.
...
best <- pcp_glance %>%
  filter(glance_adj.r.squared > 0.5) %>%
  select(composerName)
...
Modelli in cui la parabola è positiva e la componente lineare è negativa. Prendo i migliori.
...
```{r}
cs <- pcp %>%
  select(seasonN,composerName,n,totalSeason,perc)

cs %>%
  inner_join(performances)
...
performances %>%
  ...
  filter(conductorName != "Damrosch, Leopold") %>%
  ggplot() +
  geom_point(aes(seasonN,perc,color=conductorName), show.legend = FALSE)
```
